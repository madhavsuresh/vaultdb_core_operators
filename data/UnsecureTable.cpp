...
Field::Field() : is_string(false), type(FieldType::INTEGER32), field_num(0) {
  throw;
}

void UnsecureTable::put_tuple(Tuple t) { tuples.push_back(t); }

Tuple UnsecureTable::get_tuple(int index) const { return this->tuples[index]; }

void Tuple::put_field(int index, shared_ptr<Field> f) {
  fields.emplace(index, f);
}

shared_ptr<Field> Tuple::get_field(int index) { return this->fields[index]; }

UnsecureTable::UnsecureTable() = default;

int UnsecureTable::num_tuples() const { return this->tuples.size(); }

Schema *UnsecureTable::get_schema() const { return schema.get(); }
...
int Schema::get_num_fields() const { return num_fields; }

FieldDesc *Schema::get_field(int i) const { return this->fields.at(i).get(); }
...
Field::Field(FieldType t, int64_t val, int fn)
    : is_string(false), type(t), field_num(fn) {
  value.int64Value = val;
}

Field::Field(FieldType t, double val, int fn)
    : is_string(false), type(t), field_num(fn) {
  value.doubleValue = val;
}

Field::Field(FieldType t, int32_t val, int fn)
    : is_string(false), type(t), field_num(fn) {
  value.int32Value = val;
}

Field::Field(FieldType t, const string &s, int fn)
    : is_string(true), type(t), field_num(fn) {
...
int32_t Field::get_as_int32() const { return value.int32Value; }

int64_t Field::get_as_int64() const { return value.int64Value; }

double Field::get_as_double() const { return value.doubleValue; }

Field::~Field() {
  // If deallocating string occurs here, then copy constructors will need to
  // reallocate memory
  if (is_string) {
...
int SecureTable::get_num_tuples() const { return num_tuples; }

BaseData *SecureTable::get_base_data() { return base_data.get(); }
...
std::vector<emp::Bit *> SecureTable::get_field_range(int tuple_idx,
                                                     int field_idx) const {
  return this->base_data->get_bit_range(
      tuple_idx * this->get_tuple_len() + schema->get_field_offset(field_idx),
...
int BaseData::get_length() const { return base_data.size(); }
