...
unique_ptr<QueryTable> share_data(const QueryTable *input_table, int party) {
  for (auto &t : *input_table) {
    for (auto &field : t) {
    }
  }
}
...
unique_ptr<SecureTable> share_data(const UnsecureTable *input_table,
  ...
  for (int i = 0; i < alice_size; i++) {
    for (int j = 0; j < input_table->get_schema()->get_num_fields(); j++) {
      if (party == emp::ALICE) {
        ...
        assert(bits.size() ==
               input_table->get_schema()->get_field(j)->get_field_size());
        for (int c = 0;
             c < input_table->get_schema()->get_field(j)->get_field_size();
             c++) {
  ...
  std::cout << "Alice (" << batcherAlice->size() << "), (" << batcherBob->size()
            << std::endl;
