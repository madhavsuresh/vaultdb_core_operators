...
QueryField::QueryField(QueryField &qf) : field_num(qf.field_num), type(qf.type), is_string(qf.is_string),
...
QueryField::QueryField(int64_t val, int fn) : is_string(false), type(FieldType::INTEGER64), field_num(fn) {
    unencrypted_value.int64Value = val;
}

QueryField::QueryField(int32_t val, int fn) : is_string(false), type(FieldType::INTEGER32), field_num(fn) {
    unencrypted_value.int32Value = val;
}

QueryField::QueryField(double val, int fn) : is_string(false), type(FieldType::DOUBLE), field_num(fn) {
    unencrypted_value.doubleValue = val;
}

QueryField::QueryField(std::string val, int fn) : is_string(true), type(FieldType::VARCHAR), field_num(fn) {
